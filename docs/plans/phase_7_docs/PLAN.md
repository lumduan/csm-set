...
**Status:** `[~]` Verify-and-lock (verify the 742 tests / 92% claim, then lock the floor)
...
- [ ] Run the full suite locally: `uv run pytest tests/ -v --cov=api --cov-report=term-missing` and capture the actual numbers (test count, line coverage on `api/`).
- [ ] If coverage on `api/` is < 90%, identify the top three uncovered modules and add targeted tests; if ≥ 90%, proceed to lock.
- [ ] `pyproject.toml` modification — add `--cov-fail-under=90` to `[tool.pytest.ini_options].addopts` (or set in `[tool.coverage.report].fail_under = 90`).
- [ ] Confirm the suite still passes with the floor in place: `uv run pytest tests/`.
- [ ] Document the actual numbers (test count, coverage %, date) in the Completion Notes section of this sub-phase.
- [ ] Confirm no `tests/` files contain network calls in the unit-test layer (project rule from `.claude/knowledge/project-skill.md`).
...
**Status:** `[x]` Code complete (Phase 5.7); `[ ]` Documentation pending
