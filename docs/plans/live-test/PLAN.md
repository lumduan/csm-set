...
3. **Portfolio Update** — If it's a **rebalance day (last trading day of month)**: `ExecutionSimulator.simulate()` generates trade list (sells + buys). Holdings are evicted if: (a) below 35th percentile exit floor, (b) a challenger ranks 25+ percentile points higher (buffer logic), or (c) price < EMA100 (fast exit). Execute at ATO on the **first trading day of next month**. Other days: portfolio marked to market at latest closing prices. No intra-month trading.
