...
`commission_cum` are unchanged (no trade since the 2026-08-03 rotation; the 2026-09-01 rebalance
traded nothing), so the whole movement is the unrealized leg (+301,442.35 → **+283,638.35**, a
**−17,804.00** session — the largest decline since 2026-09-16, checked against
`db_gateway.daily_performance` over the whole series). `total_pl` falls to **+234,546.97**, 17,804.00
below the +252,350.97 maximum of 2026-09-23 and above every earlier session. The reconciliation
residual holds at **1,610.27**. **`pnl_realized_unrealized.png` was regenerated in the same
session**, so the two artifacts end together at **98 points**; the four PNGs were checksummed before
and after and the three NAV charts are byte-identical (`drawdown` eea580c3…, `equity_curve`
25b0baf9…, `monthly_returns` aebd96bd…), with only the P/L chart's hash moving (3e0f4f49… →
**254074fd…**). Every point in the series remains **pre-registration history — seen, not scored**
under the operator decision of 2026-09-11; no scoring criteria have been pre-registered since.

**`pnl.csv` extended through 2026-09-23** on 2026-09-23, appending the **2026-09-23** row — one row,
continuing the same-session practice for a **twenty-ninth** consecutive session. `realized_cum` and
`commission_cum` are unchanged (no trade since the 2026-08-03 rotation; the 2026-09-01 rebalance
traded nothing), so the whole movement is the unrealized leg (+276,570.35 → **+301,442.35**, a
**+24,872.00** session). 🟢 **`total_pl` extends its maximum to +252,350.97**, now 40,915.00 above
the +211,435.97 of 2026-08-18, on a third consecutive NAV high, and **`unrealized` sets the series
maximum at +301,442.35** — the first value above 300,000 in a series that starts 2026-05-05. The
reconciliation residual holds at **1,610.27**. **`pnl_realized_unrealized.png` was regenerated in the
same session**, so the two artifacts end together at **97 points**; the four PNGs were checksummed
before and after and the three NAV charts are byte-identical (`drawdown` eea580c3…, `equity_curve`
25b0baf9…, `monthly_returns` aebd96bd…), with only the P/L chart's hash moving (0019c396… →
**3e0f4f49…**). Every point in the series remains **pre-registration history — seen, not scored**
under the operator decision of 2026-09-11; no scoring criteria have been pre-registered since.

**`pnl.csv` extended through 2026-09-22** on 2026-09-22, appending the **2026-09-22** row — one row,
continuing the same-session practice for a **twenty-eighth** consecutive session. `realized_cum` and
`commission_cum` are unchanged (no trade since the 2026-08-03 rotation; the 2026-09-01 rebalance
traded nothing), so the whole movement is the unrealized leg (+262,683.35 → **+276,570.35**, a
**+13,887.00** session). 🟢 **`total_pl` extends its maximum to +227,478.97**, now 16,043.00 above
the +211,435.97 of 2026-08-18, on a second consecutive NAV high; Return on NAV passed **20%** for the
first time. The reconciliation residual holds at **1,610.27**. **`pnl_realized_unrealized.png` was
regenerated in the same session**, so the two artifacts end together at **96 points**; the four PNGs
...
moving (94a55a88… → **0019c396…**). Every point in the series remains **pre-registration history —
seen, not scored** under the operator decision of 2026-09-11; no scoring criteria have been
pre-registered since.

**`pnl.csv` extended through 2026-09-21** on 2026-09-21, appending the **2026-09-21** row — one row,
continuing the same-session practice for a **twenty-seventh** consecutive session. `realized_cum` and
`commission_cum` are unchanged (no trade since the 2026-08-03 rotation; the 2026-09-01 rebalance
traded nothing), so the whole movement is the unrealized leg (+223,973.35 → **+262,683.35**, a
**+38,710.00** session — the largest single-session gain since 2026-08-17, checked against
`db_gateway.daily_performance` over the whole series). 🟢 **`total_pl` sets a new maximum at
+213,591.97**, passing the +211,435.97 of 2026-08-18 by 2,156.00 — the same margin by which NAV
passed its own peak, as it must be while `realized_cum` is constant. The reconciliation residual
holds at **1,610.27**. **`pnl_realized_unrealized.png` was regenerated in the same session**, so the
two artifacts end together at **95 points**; the four PNGs were checksummed before and after and the
three NAV charts are byte-identical (`drawdown` eea580c3…, `equity_curve` 25b0baf9…,
`monthly_returns` aebd96bd…), with only the P/L chart's hash moving (99c41350… → **94a55a88…**).
Every point in the series remains **pre-registration history — seen, not scored** under the operator
decision of 2026-09-11; no scoring criteria have been pre-registered since.

**`pnl.csv` extended through 2026-09-18** on 2026-09-18, appending the **2026-09-18** row — one row,
continuing the same-session practice for a **twenty-sixth** consecutive session. `realized_cum` and
`commission_cum` are unchanged (no trade since the 2026-08-03 rotation; the 2026-09-01 rebalance
traded nothing), so the whole movement is the unrealized leg (+224,156.35 → **+223,973.35**, a
**−183.00** session — **the smallest absolute session move of all 94 sessions of the live test**,
checked against `db_gateway.daily_performance` over the whole series). The reconciliation residual
holds at **1,610.27**. **`pnl_realized_unrealized.png` was regenerated in the same session**, so the
two artifacts end together at **94 points**; the four PNGs were checksummed before and after and the
three NAV charts are byte-identical (`drawdown` eea580c3…, `equity_curve` 25b0baf9…,
`monthly_returns` aebd96bd…), with only the P/L chart's hash moving (c2d11062… → **99c41350…**).
Every point in the series remains **pre-registration history — seen, not scored** under the operator
decision of 2026-09-11; no scoring criteria have been pre-registered since.

**`pnl.csv` extended through 2026-09-17** on 2026-09-17, appending the **2026-09-17** row — one row,
continuing the same-session practice for a **twenty-fifth** consecutive session. `realized_cum` and
`commission_cum` are unchanged (no trade since the 2026-08-03 rotation; the 2026-09-01 rebalance
traded nothing), so the whole movement is the unrealized leg (+214,657.35 → **+224,156.35**, a
**+9,499.00** session that recovers a quarter of the prior session's −38,243.00). The reconciliation
residual holds at **1,610.27**. **`pnl_realized_unrealized.png` was regenerated in the same
session**, so the two artifacts end together at **93 points**; the four PNGs were checksummed before
and after and the three NAV charts are byte-identical (`drawdown` eea580c3…, `equity_curve`
25b0baf9…, `monthly_returns` aebd96bd…), with only the P/L chart's hash moving (8d8baaf5… →
**c2d11062…**). Every point in the series remains **pre-registration history — seen, not scored**
under the operator decision of 2026-09-11; no scoring criteria have been pre-registered since.

**`pnl.csv` extended through 2026-09-16** on 2026-09-16, appending the **2026-09-16** row — one row,
continuing the same-session practice for a **twenty-fourth** consecutive session. `realized_cum` and
`commission_cum` are unchanged (no trade since the 2026-08-03 rotation; the 2026-09-01 rebalance
traded nothing), so the whole movement is the unrealized leg (+252,900.35 → **+214,657.35**, a
**−38,243.00** session — the largest single-session loss since 2026-09-01, and the third-largest of
the holding period, checked against `db_gateway.daily_performance` over the whole series). The
reconciliation residual holds at **1,610.27**. **`pnl_realized_unrealized.png` was regenerated in the
same session**, so the two artifacts end together at **92 points**; the four PNGs were checksummed
before and after and the three NAV charts are byte-identical (`drawdown` eea580c3…, `equity_curve`
25b0baf9…, `monthly_returns` aebd96bd…), with only the P/L chart's hash moving (a62d01b3… →
**8d8baaf5…**). Every point in the series remains **pre-registration history — seen, not scored**
under the operator decision of 2026-09-11; no scoring criteria have been pre-registered since.

**`pnl.csv` extended through 2026-09-15** on 2026-09-15, appending the **2026-09-15** row — one row,
continuing the same-session practice for a **twenty-third** consecutive session. `realized_cum` and
`commission_cum` are unchanged (no trade since the 2026-08-03 rotation; the 2026-09-01 rebalance
traded nothing), so the whole movement is the unrealized leg (+237,541.35 → **+252,900.35**, a
**+15,359.00** session — the largest single-session gain since 2026-09-07, checked against
`db_gateway.daily_performance`). The reconciliation residual holds at **1,610.27**.
**`pnl_realized_unrealized.png` was regenerated in the same session**, so the two artifacts end
together at **91 points**; the four PNGs were checksummed before and after and the three NAV charts
are byte-identical (`drawdown` eea580c3…, `equity_curve` 25b0baf9…, `monthly_returns` aebd96bd…),
with only the P/L chart's hash moving (0cbe13e1… → **a62d01b3…**). ↻ **Two earlier rankings in this
section were wrong when written and now carry pointers** — the 2026-09-11 and 2026-09-07 entries
below; the check that found them is recorded in `daily/2026-09-15.md`. Every point in the series
remains **pre-registration history — seen, not scored** under the operator decision of 2026-09-11;
no scoring criteria have been pre-registered since.

**`pnl.csv` extended through 2026-09-14** on 2026-09-14, appending the **2026-09-14** row — one row,
continuing the same-session practice for a **twenty-second** consecutive session. `realized_cum` and
`commission_cum` are unchanged (no trade since the 2026-08-03 rotation; the 2026-09-01 rebalance
traded nothing), so the whole movement is the unrealized leg (+250,369.35 → **+237,541.35**, a
**−12,828.00** session). The reconciliation residual holds at **1,610.27**.
**`pnl_realized_unrealized.png` was regenerated in the same session**, so the two artifacts end
together at **90 points**; the four PNGs were checksummed before and after and the three NAV charts
are byte-identical (`drawdown` eea580c3…, `equity_curve` 25b0baf9…, `monthly_returns` aebd96bd…),
with only the P/L chart's hash moving (d5c45add… → **0cbe13e1…**). Every point in the series
remains **pre-registration history — seen, not scored** under the operator decision of 2026-09-11;
no scoring criteria have been pre-registered since.
...
2026-08-17 ↻ **CORRECTED 2026-09-15: wrong when written.** Six sessions after 2026-08-17 were larger,
the nearest 2026-09-09's +14,811.00, so the correct scope is *since 2026-09-09*; checked against
`db_gateway.daily_performance`). The reconciliation residual holds at **1,610.27**. **`pnl_realized_unrealized.png` was
...
**+23,010.00** session — the largest single-session gain since 2026-08-18 ↻ **CORRECTED 2026-09-15:
wrong when written.** 2026-08-21 (+27,020.00) and 2026-08-26 (+34,934.00) were both larger, so the
correct scope is *since 2026-08-26*; checked against `db_gateway.daily_performance`). The reconciliation
residual holds at **1,610.27**. **`pnl_realized_unrealized.png` was regenerated in the same
...
`commission_cum` are unchanged (no trade since the 2026-08-03 rotation; September's rebalance was
0-out / 0-in), so the whole movement is the unrealized leg (+198,847.35 → **+183,424.35**, a
**−15,423.00** session — the largest single-session decline of the holding period). The
reconciliation residual holds at **1,610.27**. **`pnl_realized_unrealized.png` was regenerated in the
same session**, so the two artifacts end together at **83 points**; the three NAV charts were
checksummed before and after and are byte-identical (`drawdown` eea580c3…, `equity_curve` 25b0baf9…,
`monthly_returns` aebd96bd…), with only the P/L PNG's hash moving (c11bb6b2… → 2bfe465f…).

🟢 **NO restatement occurred on 2026-09-03, so no NEW divergence was added.** All ten 2026-09-02
closes still match what that log published, and the EMA100 positive control reproduced 5 of 5 —
two independent checks agreeing that the banked past did not move.

⚠️ **The 2026-09-01 divergence recorded in the previous entry PERSISTS and is still by design.**
`pnl.csv` carries the **as-published** unrealized of **+204,583.35** for that date; `equity_curve`
carries **1,273,351.70** (the FORTH-restated basis); `db_gateway.daily_performance` — append-only —
still carries **1,273,881.70**. **Three series, two bases, all three right on their own terms.** The
rule is unchanged: **`pnl.csv` tracks what was published, `equity_curve` tracks what the panel
currently says.** Do not "reconcile" them on that row.

**`nav_actual.csv` and `nav_twr.csv` were not extended** — the ordinary mid-month state; they are
rebuilt from `equity_curve` **at month-end**, so the four PNGs sit on two dates as usual. ⚠️ The
next month-end rebuild will pick up the restated 2026-09-01 bar, which is intended.
