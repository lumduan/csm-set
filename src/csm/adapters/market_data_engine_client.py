...
lets csm-set *read* pre-fetched bars from its auth-gated read API
(``GET /ohlcv`` and ``GET /ohlcv/adjusted``) instead of fetching tvkit
directly — used when ``CSM_OHLCV_SOURCE='db'`` (feature-market-data-engine
Phase 3).

The client owns one shared :class:`httpx.AsyncClient`, retries on transient
5xx and transport errors with bounded backoff, surfaces 4xx as terminal
errors, parses the wire's ``Decimal``-as-string prices into
:class:`~decimal.Decimal`, and supports a custom transport for testability.
It holds **no tvkit credential** — only an optional ``X-API-Key`` for the
engine's read API.
"""
...
class MarketDataEngineError(Exception):
    """Raised when an engine read fails after all retries or returns a 4xx."""
...
class MarketDataEngineClient:
    ...
    Args:
        base_url: Engine read-API base URL (without the path), e.g.
            ``http://quant-marketdata-engine:8000`` in-cluster or
            ``http://localhost:8300`` for host-local dev.
        api_key: Optional shared secret sent as ``X-API-Key``. The engine
            only enforces it when its own ``MARKETDATA_ENGINE_API_KEY`` is set.
    ...
    async def get_ohlcv(
        ...
        *,
        ...
        ``/ohlcv``. Retries transient 5xx / transport failures with backoff;
        4xx (e.g. 401 auth, 422 bad params) are terminal.

        Args:
            symbol: Instrument symbol, e.g. ``"SET:PTT"``.
            timeframe: Engine timeframe — one of ``"1d"``, ``"1h"``, ``"5m"``.
            adjusted: When True, request dividend/split adjust-on-read bars.
