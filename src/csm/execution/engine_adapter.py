...
``ExecutionEngineAdapter`` mirrors :class:`csm.adapters.gateway_client.GatewayClient`'s
construction (base_url, api_key, transport injection, retry/backoff) and adds:
...
from csm.execution.models import NormalizedOrder, NormalizedOrderResult, OrderUpdateEvent
...
class ExecutionEngineAdapter:
    ...
    Args:
        ...
        strategy_id: Slug sent as ``X-Strategy-Id`` and used as the default
            stream filter. Defaults to ``"csm-set"``.
