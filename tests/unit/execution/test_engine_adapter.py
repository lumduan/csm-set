"""Unit tests for :mod:`csm.execution.engine_adapter` (Phase 5.1)."""
...
from csm.execution.models import NormalizedOrder
...
def _event_frame(
    seq: int, state: str, *, fill_qty: int | None = None, fill_price: str = "35.55"
) -> str:
    data: dict[str, Any] = {
        "seq": seq,
...
class TestStreamUpdates:
    ...
    async def test_happy_parse_yields_events(self) -> None:
        body = _sse(
            _event_frame(1, "NEW"),
            _event_frame(2, "FILLED", fill_qty=100),
        )
    ...
    async def test_keep_alive_comments_ignored(self) -> None:
        body = (": keep-alive\n\n" + _event_frame(1, "FILLED", fill_qty=100) + "\n\n").encode()
        seq = _Sequencer([_sse_response(body)])
        async with ExecutionEngineAdapter(
            base_url=BASE_URL, api_key=API_KEY, transport=httpx.MockTransport(seq)
        ) as adapter:
    ...
    async def test_gap_advisory_warns_and_continues(self) -> None:
        body = (
            'event: gap\ndata: {"dropped": 3}\n\n'
            + _event_frame(5, "FILLED", fill_qty=100)
            + "\n\n"
        ).encode()
        seq = _Sequencer([_sse_response(body)])
        async with ExecutionEngineAdapter(
    ...
    async def test_reconnect_sends_last_event_id_and_skips_duplicate_seq(self) -> None:
        ...
        second = _sse_response(
            _sse(_event_frame(1, "NEW"), _event_frame(2, "FILLED", fill_qty=100))
        )
    ...
    async def test_connected_event_set_on_successful_open(self) -> None:
        ...
        body = _sse(_event_frame(1, "FILLED", fill_qty=100))
        seq = _Sequencer([_sse_response(body)])
